...
public class GetRecipeByIdInvalidTokenTest(CustomWebApplicationFactory factory) : MyRecipeBookClassFixture(factory)
{
    ...
    public async Task Error_Token_Invalid(string culture)
    {
        ...
        var response = await DoGet(method: $"{METHOD}/{id}", token: "invalid_token", culture: culture);
    ...
    public async Task Error_Without_Token(string culture)
    {
        ...
        var response = await DoGet(method: $"{METHOD}/{id}", token: string.Empty, culture: culture);
